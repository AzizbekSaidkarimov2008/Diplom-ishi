function sub() {
  let b = document.mal.roziman;
  if (b.checked) {
    document.mal.tugma.disabled = false;
  } else {
    document.mal.tugma.disabled = true;
  }
}
/** GLOBAL OBJECT **/
var $ = {
  ...
  selectCreate: function (select, label) {
    ...
    input.addEventListener("focus", function () {
      ...
      populateList(
        this,
        select,
        document.getElementById("ul" + this.id),
        -1,
        true
      );
    }); // END focus
    ...
    function getList(val, list, focus) {
      ...
      for (var i = 0, x = list.length; i < x; i++) {
        if (
          list[i].text !== "" &&
          (list[i].text.toUpperCase().substring(0, vLength) ===
            val.toUpperCase() ||
            list[i].value.toUpperCase().substring(0, vLength) ===
              val.toUpperCase())
        ) {
